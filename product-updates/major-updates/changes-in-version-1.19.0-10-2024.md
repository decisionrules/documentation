...
Date cell is a specific kind of cell accompanying the [date operators](../../rules/data-types-and-functions/operators/date-operators.md). It allows to enter dates in order to build a date condition. Up to now, date cells featured a date picker. It was not able to copy paste the dates or write them in a text form.
