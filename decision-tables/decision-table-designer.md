...
{% hint style="info" %}
More information is in the [Binding To Model](input-and-output/binding-to-model.md).
{% endhint %}
...
Each column has a column alias, a unique identifier within the scope of the table. You can use the alias to reference the column when using the [included/excluded condition column](../api/rule-solver-api.md#included-condition-cols) feature or when binding column [valid values](table-operations/valid-values.md) in an [XLSX file](import-and-export-rule/managing-decision-table-in-excel-google-sheets.md#valid-values).
...
A row represents one rule for specified data. If it's called, the [Rule Solver](../api/rule-solver-api.md) or the [Test Bench](../other/test-bench.md) with Input Data corresponding to **conditions** in a row, the[ Rule Solver](../api/rule-solver-api.md) or the [Test Bench](../other/test-bench.md) response will have the same data specified in **results** in the corresponding row.
...
{% hint style="info" %}
Possible operators are specified in [Operators](operators/) section
{% endhint %}
...
Read more in [Math function](operators/functions/math.md#minus) section.
{% endhint %}

#### Values

{% hint style="info" %}
Possible values for data types are specified in [Data Types](data-types.md) section.
{% endhint %}
